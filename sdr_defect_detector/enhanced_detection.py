...
def refine_element_detection(image, suspicious_positions, element_model, element_threshold):
    ...
    for pos, window_conf in suspicious_positions:
        try:
            ...
            if window_conf > element_threshold * 1.2:  # Higher threshold for fallback
...
def detect_edge_defects(image, element_model, element_threshold):
    ...
    for pos in range(norm_image.shape[1] - edge_width, norm_image.shape[1]):
        if pos >= 0 and pos < norm_image.shape[1]:
            features = simple_element_features(norm_image, pos)
            element_proba = element_model.predict_proba([features])[0, 1]
...
def detect_multiple_defects(image, positions, element_model, element_threshold):
    """Specialized detection for multi-element defects"""
    if not positions:
        return []
    ...
    def group_by_proximity(positions, max_gap=2):
        ...
        for pos in sorted_pos[1:]:
            if pos - current_group[-1] <= max_gap:
    ...
    # Get position values without confidences
    pos_values = [pos for pos, _ in positions]
    
    # Group detected positions
    groups = group_by_proximity(pos_values)
    
    # For each group with >1 element, check surrounding elements
    additional_detections = []
    
    for group in groups:
        if len(group) > 1:
            # Group of multiple detections - check elements between and adjacent
            min_pos = min(group)
            max_pos = max(group)
            
            # Check elements within the range plus one element on each side
            check_range = range(max(0, min_pos-1), min(norm_image.shape[1], max_pos+2))
            
            for pos in check_range:
                if pos not in group and pos < norm_image.shape[1]:
                    # Check if this should also be detected
                    features = simple_element_features(norm_image, pos)
                    element_proba = element_model.predict_proba([features])[0, 1]
                    
                    # Use lower threshold for elements adjacent to known defects
                    if element_proba >= element_threshold * 0.8:
                        additional_detections.append((pos, element_proba))
...
def apply_enhanced_post_processing(image, defect_positions):
    """Enhanced post-processing with better handling of edge and multiple defects"""
    ...
    for pos, conf in defect_positions:
        ...
        # More lenient threshold for edge elements
        ...
        # Check for isolation and clustering
        isolation_distances = []
        for other_pos, _ in defect_positions:
            if other_pos != pos:
                isolation_distances.append(abs(other_pos - pos))
        
        is_isolated = not isolation_distances or min(isolation_distances + [999]) > 2
        
        # Check for cluster patterns (multiple adjacent defects)
        is_part_of_cluster = False
        if not is_isolated:
            # Count nearby detections
            nearby_count = sum(1 for d in isolation_distances if d <= 2)
            # If multiple adjacent detections, likely a real multi-element defect
            is_part_of_cluster = nearby_count >= 2
        
        # Analyze vertical pattern
        min_vertical_intensity = 1.0
        if pos < image.shape[1] and image.shape[0] > 0:
            element_col = image[:, pos]
            if len(element_col) > 0 and np.max(element_col) > 0:
                min_vertical_intensity = np.min(element_col) / np.max(element_col)
        
        # Final decision with multiple criteria
        keep_detection = (
            # Must meet at least one physical criterion
            (is_local_minimum or min_vertical_intensity < 0.3 or is_at_edge) and
            # Must meet confidence criterion (more lenient for edges)
            (conf > 0.7 * edge_threshold_factor) and 
            # Allow both isolated AND clustered patterns
            (is_isolated or is_part_of_cluster or conf > 0.9 or is_at_edge)
        )
...
def enhanced_detection(image, window_model, window_threshold, 
                     element_model=None, element_threshold=None,
                     window_size=8, overlap=0.5):
    """Enhanced detection addressing edge and multiple defect issues"""
    ...
    # Stage 3: Apply enhanced post-processing
    main_positions = apply_enhanced_post_processing(image, refined_positions)
    
    # Stage 4: Special detection paths
    if element_model:
        # Add edge detections
        edge_detections = detect_edge_defects(image, element_model, element_threshold)
        
        # Add multiple defect detections
        if len(main_positions) >= 1:
            multiple_detections = detect_multiple_defects(
                image, main_positions, element_model, element_threshold
            )
            
            # Combine all detections
            all_positions = list(main_positions)
            
            # Add edge detections
            detected_positions = set(pos for pos, _ in all_positions)
            for pos, conf in edge_detections:
                if pos not in detected_positions:
                    all_positions.append((pos, conf))
                    detected_positions.add(pos)
            
            # Add multiple defect detections
            for pos, conf in multiple_detections:
                if pos not in detected_positions:
                    all_positions.append((pos, conf))
                    detected_positions.add(pos)
            ...
            # Just combine main and edge detections
            all_positions = list(main_positions)
            detected_positions = set(pos for pos, _ in all_positions)
...
def main():
    parser = argparse.ArgumentParser(description='Enhanced Defect Detection')
    parser.add_argument('--window_model', type=str, required=True, help='Window-level model file')
    parser.add_argument('--element_model', type=str, help='Element-level model file')
    parser.add_argument('--data_file', type=str, required=True, help='HDF5 data file')
    parser.add_argument('--output_dir', type=str, default='enhanced_output', help='Output directory')
